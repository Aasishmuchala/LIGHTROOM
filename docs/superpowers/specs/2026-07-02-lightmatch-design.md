...
- **Exactness target (user requirement): 99% similar.** The loop's goal is a near-duplicate of the reference's lighting, not a vibe. The loop continues until the convergence score enters the tight-match band (≤5 on the 0–100 scale, threshold calibrated against the user's judgment during stage 2) or a structured grade-handoff is declared. The final 99% end-state is the two-tool pipeline: LightMatch nails light transport, REFGRADE nails the residual color distribution.
...
- **Acceptance (v1 done means):**
  1. On three of the user's real scenes, the refine loop (≤5 rounds) reaches the tight-match band (score ≤5) **or** a structured grade-handoff whose residual REFGRADE then closes — with the end result judged ≥99% similar to the reference by the user, and the score improving monotonically in at least two of the three scenes.
